...
def prepare_clinical_encoder(metadata_df):
    ...
    one_hot_encoder.fit(metadata_df[['sex', 'localization']])

    age_scaler = StandardScaler()
    age_scaler.fit(metadata_df[['age']])
...
def extract_clinical_features_from_list(file_names, metadata_df, one_hot_encoder, age_scaler):
    ...
    # Encode categorical + normalize age
    cat_feats = one_hot_encoder.transform(matched_df[['sex', 'localization']].fillna("unknown"))
    age_feat = age_scaler.transform(matched_df[['age']].fillna(age_scaler.mean_[0]))  # ✅ fix here
...
def predict_labeled_test_set(model_path, label_encoder_path, test_paths, feature_types, result_dir, categories,
    ...
    for feature_type in feature_types:
        if first_feature:
            ...
            X_part, _ = load_features_without_smote(test_paths, feature_type, categories, return_filenames=False)
...
def main():
    ...
    clinical_metadata_path = os.path.join(home_dir, "HAM10000_metadata.csv")
    feature_dir = os.path.join(home_dir, 'data2_SOTA_and_handcrafts_and_BlookNet_optimal_entropy_features')
    # feature_dir = os.path.join(home_dir, 'data_balanced_small_max50')
    
    result_folder = os.path.join(home_dir, 'training_data2_Multi_modal_GNN_Skin_Lesion_Classification_v1')
    os.makedirs(result_folder, exist_ok=True)
    ...
    # 📋 Load metadata
    metadata_df = pd.read_csv(clinical_metadata_path)
    one_hot_encoder, age_scaler = prepare_clinical_encoder(metadata_df)
